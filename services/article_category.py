...
def fetch_10_random_categories():
...
def fetch_number_of_article_titles(category: str, number_of_articles: int):
    articles = []
    while True:
        search_results = wikipedia.search(category, results=number_of_articles * 10)
        random_search_results = random.sample(search_results, number_of_articles)
        for result in random_search_results:
            page = wiki_wiki.page(result)
            if page.exists():
                if page.title not in articles and len(page.text) > 0:
...
def create_summary_of_selected_hidden_article(random_article_title: str):
    ...
    max_length = 1000  # fix length for the summary
    if page.exists():
        summary = page.summary
        # Trim to the desired length without cutting off mid-word
...
def create_10_articles_choices(category: str, hidden_article_title: str):
    choices_articles = fetch_list_of_random_9_articles_titles(category)
